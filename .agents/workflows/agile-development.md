...
#### Luồng hoàn chỉnh mỗi task:
```
⬜ To Do → 🔄 In Progress (Bước 2.2: Code) → 📦 Committed (Bước 2.3: Commit & Push) → 🔍 Review (Bước 2.4) → ✅ Done
```

> **Lưu ý:** Nếu task lớn, có thể chia thành nhiều commit nhỏ (atomic commits). Mỗi commit phải có ý nghĩa độc lập và build được.

### Bước 2.4: Code Review
